...
The worker depends only on :class:`RoomClient`. Phase 1 ships
:class:`FakeRoomClient` (in-process, no server) for dev/tests; the real
LiveKit-backed client is created lazily by :func:`build_room_client` so that
``import rtva`` never requires the ``livekit`` SDK to be installed (rule 12:
import must be pure, cheap, and total).
"""
