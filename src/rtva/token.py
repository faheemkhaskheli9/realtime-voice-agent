...
LiveKit access tokens are JWTs signed with the project API secret, carrying a
``video`` grant claim. This is enough for a test client (or the agent worker)
to join a room. Kept dependency-light (PyJWT only) rather than pulling the
full server SDK.
"""
