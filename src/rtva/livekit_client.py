"""Real LiveKit-backed :class:`~rtva.room.RoomClient`.

Imported lazily by :func:`rtva.room.build_room_client` only when credentials
are present, so the ``livekit`` SDK stays an optional dependency for anyone
running the Phase 1 dry-run path.
...
real LiveKit server (see ``scripts/join_smoke.py``), not in unit tests.
"""
